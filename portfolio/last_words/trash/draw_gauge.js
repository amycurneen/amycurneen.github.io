...
var arc = d3.svg.arc()
  .innerRadius(iR)
  .outerRadius(oR)
  .startAngle(-90 * (pi/180)); // Arc Defaults
...
var background = svg.append("path").attr("class", "gauge")
  .datum({endAngle:  90 * (pi/180)})
  .style("fill", "#ddd")
  .attr("d", arc);// Append background arc to svg

var foreground = svg.append("path").attr("class", "gauge")
  .datum({endAngle: -90 * (pi/180)})
  .style("fill", cur_color)
  .attr("d", arc); // Append foreground arc to svg
...
setInterval(function() {
  ...
  current.transition()
    .text(Math.floor(num));// Text transition
